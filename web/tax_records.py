...
finance_mails_char_id = check_mailer_token()
...
try:
    ...
    logger.info('check_taxes: could not connect to database %s', err)
    sys.exit()
...
def select_taxable_corps() -> list[dict]:
...
def main():
    ...
    for tax_record in tax_records:
        ...
        if (
            ...
            body, recipients, mail_subject = prepare_tax_mail(tax_record, config)
            post_tax_mail(
